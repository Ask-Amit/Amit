...
**Known gap against this standard, honestly (not yet fixed):** most existing list screens (Contacts, Items, Fixed Assets, Payments, and the rest of the Phase 2 panels) do not yet use the `abSmart*` engine or have Print Preview wired in — Bills & Invoices and Trial Balance are the only two converted so far (2026-07-31). Retrofitting the rest is real, tracked work (see the To Do tab in the app), not silently claimed as done here.
