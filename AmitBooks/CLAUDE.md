...
**Current coverage (updated 2026-07-31):** the `abSmart*` engine is wired into every real ledger-style table — Bills & Invoices, Payments, Contacts, Fixed Assets, Items, Tags/Jobs, Purchase Orders, Inventory, Payroll (employees + pay runs), Mileage, Repairs, Warranties, Service Agreements, Investments, Subscriptions — and Print Preview covers Trial Balance, P&L, and Balance Sheet. **Deliberately left as-is, not gaps:** Rental Properties and CRM Activity are card/timeline views where a sortable-table retrofit wouldn't add real value (a handful of properties; a chronological activity log is already in its natural order); the Review Queue is an action worklist meant to be worked through, not browsed. **Genuine remaining gap:** most screens still lack the double-click cross-link into Contacts/Jobs — only Bills & Invoices, Payments, Purchase Orders, and Tags/Jobs have it so far (see rule 5 above).
