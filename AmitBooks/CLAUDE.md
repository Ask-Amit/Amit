...
**5. Everything cross-links to its own detail, everywhere it's shown, not just within its own screen.** An entity name shown inside a different screen (an account name inside a Journal line, a contact name inside a bill) opens straight into that entity's own record — added 2026-07-31 after Ryan named it directly: "everything that's visible on the screen needs to be connected to the other parts and pieces." Three generic functions cover the three entity types named throughout the app: `abOpenAccountInChartOfAccounts(id, name)`, `abOpenContact(id, name)`, `abOpenJob(id, name)` — each switches to the right tab and registers an Open Pages entry. **Trigger revised 2026-07-31 (Ryan's direct correction, supersedes the original "double-click opens the record" wording above and in earlier build notes):** double-clicking a name VALUE inside a table row now filters that table down to just that value (`abSmartFilterByValue(tableKey, col, value)`, in the same `abSmart*` engine as column-header filtering) — the fast "show me everything for this one vendor/account/job" shortcut, same double-click gesture Ryan already uses for column-header filters, applied consistently to row values too. Opening the underlying record moves to its own explicit small trigger next to the name (a "→" link) instead of sharing double-click with two different meanings. **Rolled out so far: Bills & Invoices contact name.** Still on the old double-click-opens-record behavior and needs the same conversion: Payments, Purchase Orders, Tags & Jobs, Journal line detail (account name), and everywhere else `abOpenContact`/`abOpenAccountInChartOfAccounts`/`abOpenJob` are wired via `ondblclick` — this is real, tracked follow-up work, not finished everywhere yet.
