...
**Live, in active development — v1.33 as of 2026-07-30.** Real Supabase backend, real double-entry journal auto-posting, real Reports (Trial Balance/P&L/Balance Sheet), a Simple/Visual toggle on every screen (document-shaped views — a real check-stub checkbook, stamped bill/invoice/PO cards, pay stubs, rolodex cards), and a drill-down General Journal viewer (see Interactivity Standard below). Not yet wired to real bank feeds, e-filing, or receipt OCR.
...
**Why this is a standing rule and not a one-off:** Ryan's direct instruction, 2026-07-30 — "anything that is displayed on the screen should be able to be drilled down into the detail levels at all times... every form, every log, every journal should be able to be sortable... without having to come back and redevelop it all the time." The goal is that every future screen in AmitBooks is built to this bar the first time, reusing the Journal viewer's drill/sort/select/audited-delete pattern rather than each screen reinventing its own weaker version.
