...
**2026-09-05 — New `/api/claude-status` endpoint + installer Claude-connect step (built for AmitMobile/Hub's readiness-check work).** `amit_bridge_server.ps1` gained a read-only `/api/claude-status` route reporting `{installed, connected}` for the Claude Code CLI (NOT Claude Desktop — a separate, unrelated Anthropic product not used anywhere in this mechanism) on this machine. Determined entirely from on-disk evidence — `claude` on PATH or `%APPDATA%\npm\claude.cmd` present (installed); `%USERPROFILE%\.claude\.credentials.json` present with a real `claudeAiOauth.accessToken` and an unexpired `refreshTokenExpiresAt` (connected) — never a live `claude auth`/`claude -p` call, to avoid the documented risk of disrupting another open Claude Code session on the same machine. Both `Install_AmitTracker.ps1` copies (`Watchers\` and `Watchers\AmitInstaller\`) gained a new Step 1c that re-checks install state (Step 1b already auto-installs Node + the CLI) and, if installed but not signed in, offers (via `Read-Host`, never forced) to run `claude login` right there. Full detail and the shared JS consumer (`amit_readiness_check.js`) in `AmitMobile\CLAUDE.md`'s "Readiness Check" section. Not yet re-synced to the installed runtime copy at `%LOCALAPPDATA%\AmitComputerHealth\Watchers\` — same open item as every other bridge change this week.
