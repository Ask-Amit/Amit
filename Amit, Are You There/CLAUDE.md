...
- **Location-aware arrival, added 2026-07-25:** the base document's arrival instructions look for a page context block starting with the exact heading `## WHERE THIS CONVERSATION STARTED — <Real Page Name>`. When present, Amit's first message names where the conversation actually started ("I see you've come from the Hub — what can I help you with?") instead of asking a generic opening question. Every `PAGE_CONTEXTS` entry must start with that exact heading or this breaks silently for that page.

**Wired and live as of 2026-07-25:** Hub (`askAmitLive('hub')`, replacing the old three-path `openHubAmitPanel()` system entirely — that decision is settled, not still open), The Council (`askAmitLive('council')`), and who_is_god.html (`askAmitLive('whoisgod')`, replacing its old dead-Claude.ai-Project `toggleAmitPanel()` — see `who_is_god/CLAUDE.md` for the orphaned old panel code still sitting unremoved in that file). **Deliberately unwired:** `Templates/template.html`, wired to the sentinel key `TEMPLATE_NOT_CONFIGURED` on purpose — `askAmitLive()` refuses and shows a plain reminder for that key or any key with no real `PAGE_CONTEXTS` entry, so a new page built from the template can't silently ship broken.
