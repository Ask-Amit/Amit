...
- **Page icons resized to match the header (32px), not forced square, plus hover tooltip (added 2026-08-06, Ryan's direct instruction).** `.page-icon` was 26x18 — too small next to the header's 32px Amit logo. Now 32x32, no border-radius/clip-path applied, so a round icon (or any shape) renders exactly as its source image is when a real project swaps one in. Hovering a page — its icon when the sidebar is collapsed, or anywhere on the row when expanded — shows that page's one-line description (the same text as `.page-sub`, written once via a `subText` variable and reused in both places so it can't drift into two different copies). Built as a single JS-positioned (`fixed`) tooltip element (`#pageHoverTip`, `initPageTooltips()`), not pure CSS, because `.sidebar` scrolls vertically (`overflow-y:auto`) — per the CSS spec, setting overflow on one axis silently forces the other axis to `auto` too, which would clip a CSS-positioned tooltip trying to extend past the sidebar's right edge. A single fixed element sidesteps that. Live-tested 2026-08-06: icon confirmed 32x32, tooltip confirmed showing correct text and hiding on mouseout in the expanded sidebar, AND confirmed showing unclipped in the collapsed icon-only view — that second case is the one that would have silently failed with a naive CSS approach.
