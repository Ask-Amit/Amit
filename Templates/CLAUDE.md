...
- **Page icon x-position fixed, identical in collapsed and expanded sidebar (added 2026-08-06, Ryan's direct instruction).** The icon used to land at a different horizontal position in each state — collapsed used `justify-content:center` with different padding math than expanded's left-aligned row, so the icon visibly shifted when the sidebar toggled. Fixed by giving both states the same left padding (11px) and left-aligning collapsed too (`justify-content:flex-start`), instead of centering it; collapsed sidebar width bumped 60→68px so the icon still fits without clipping. Live-tested 2026-08-06: icon's left-edge x-coordinate confirmed byte-identical (22px) in both states, zero px difference, icon fully within the collapsed sidebar's bounds.
