...
- **Alt+N badge — dev/reference aid only, never meant to ship live (added 2026-08-04, revised same day, Ryan's direct instruction).** First version put the page number directly inside the icon slot, replacing it — Ryan corrected this: the icon placeholder (`▢`, or a real icon once a project sets one) must always stay visible on its own, so the shortcut hint is a separate small `.page-alt-badge` element next to it, reading "Alt" + the number (e.g. "Alt 3"), matching the Alt+N keyboard shortcut below it. Controlled by `DEV_SHOW_PAGE_NUMBERS` (near `PAGE_COUNT`) — `true` while working locally/in AmitCoder, must be set `false` before any real project actually goes live (hides the badge only; the icon slot is unaffected either way). The underlying page number never changes regardless of the badge's visibility. Documented in `template.html`'s own `NOMENCLATURE` comment too.
- **Alt+number page shortcuts (added 2026-08-04, Ryan's direct instruction).** Alt+1 jumps to page 1, Alt+20 to page 20, scaling automatically with `PAGE_COUNT` — no per-page wiring needed as pages are added or removed. Two-digit pages need both digits pressed while Alt stays held (Alt+2, Alt+0); digits are buffered on `keydown` and committed to `openPage()` on Alt's `keyup`, since the browser delivers each digit as its own event. Skipped while focus is in an input/textarea so it never steals a real keystroke (e.g. Row View's filter box). Nomenclature unaffected — still "page," this is just a fast way to reach one.
