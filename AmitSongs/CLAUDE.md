...
"You're in AmitSongs, not the main Amit folder. Please close this and reopen VS Code at `C:\Users\user1\OneDrive\Documents - onedrive\Amit\` — that's where all development happens. Nothing has been built yet; this is just a heads-up before we start."
...
## Folder Confirmation
If you are reading this file, you are in: `C:\Users\user1\OneDrive\Documents - onedrive\Amit\AmitSongs\`
All AmitSongs development files belong here. Do not create AmitSongs files anywhere else. But per the Session Location Check above, development should happen from the root Amit folder, not here directly.
...
`hub_entries` (pursuits stamped `program='AmitSongs'`), `template_pages` (if the Add Page mechanism is ever used here) — same shared tables as every other project, no dedicated Songs tables yet.
...
This project's canonical name, for any pursuit created from within it, is: **AmitSongs**

Any pursuit written to `hub_entries` from this project must be stamped `program='AmitSongs'` — automatically, by this project's own code or by Amit acting on its behalf, using this exact spelling every time. Never ask the person creating the pursuit *what* program a specific pursuit belongs to — that's always this project's own name, decided once, not per-item.
...
Part of the wider mission of drawing people toward Yeshua — music as another on-ramp alongside the Hub's daily companion experience and who_is_god.html's evidence work. Ryan has said this will grow into a real tab-view system organized by song/event (certain songs tied to certain feast days), but that fuller structure is future work, not tonight's scope.

## Current Status

In development — Session 1 (2026-09-11). First song ("Blow the Shofar") and its landing page being built now.
...
Linked from the Hub's left sidebar (a "Songs" tile between Amit's Prayer and Mail, added 2026-09-11) via `../AmitSongs/AmitSongs.html`. Shares the same Amit identity, Supabase backend, and `Amit_Ask_Live.js` Ask Amit mechanism as every other project.
