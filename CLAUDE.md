...
**Built as its own standalone, reusable file — Ryan's direct instruction, same day: "make it its own separate mechanism... someone just pulls it, looks at it, understands it, and you can reuse it."** The real mechanism is `amit_pin_encryption.js`, at the Amit root — `amitPinEncode`/`amitPinDecode`/`amitPinSelfEncode`/`amitPinVerify`, pure/synchronous, no DB access of its own. Any project pulls it in with `<script src="../amit_pin_encryption.js"></script>` (path adjusted per depth) and calls those functions directly — never reconstruct the math from memory, never hand-copy it inline into a new file. `AmitBooks\NEW.html` (Outstanding page's Access field) and `Hub\amit-hub.html` (About Me → Set/Change Access PIN) are both real, live callers of this same shared file — read either one for a working usage example, including how to resolve the caller's own storage (they each use `amit_owner_contact.js`'s `getOrCreateOwnerContact`/`saveOwnerContactFields` to read/write `contacts.access_pin_check`, since PIN Encryption itself has no opinion on where a caller stores anything).

**The math, exact:** the PIN/passphrase can be any characters — numbers, letters, symbols, any mix — minimum 4 characters (longer/more varied is stronger). Each character contributes a shift digit: a numeric character uses its own value; any other character uses its char code mod 10. Every shift digit then runs through `(d+5)%10` (Ryan's own correction from an old personal password scheme — this transform is fixed, not optional). Those transformed digits become a repeating key: character *i* of the value being protected is shifted by `shifts[i % shifts.length]` — added on encode, subtracted on decode. Punctuation/dashes in a value (e.g. a formatted SSN) are just more ASCII characters to the cipher — no special-casing needed either way.

**What's actually stored, and what never is:** the real PIN is NEVER written to the database in any form, anywhere — it lives only in the person's memory and in a JS variable for the current browser session (cleared on reload). What IS stored: (1) the encoded value itself (e.g. `access_encrypted` — the protected data, shifted by the PIN); (2) a verification checksum (`access_pin_check`) — the PIN encoded using ITSELF as the key, so the only way to reproduce that stored value is to already know the real PIN. Before trusting any entered PIN to decode real data, check it against this stored checksum first — wrong PIN gets a clear "that's not the right PIN," never a silent garbage decode. A separate plain-text reminder hint (`access_pin_hint` — "help me remember what I wrote," Ryan's own words, NOT a real identity-verification security question) may also be stored, purely as a memory aid for the person, never wired into the cipher math itself. If the PIN is ever lost, there's no automatic reset — Ryan's stated future plan is a manual email-based workaround, not yet built.

**Honest limitation, on record, not to be silently forgotten:** this is a real, reversible cipher, genuinely far weaker than the existing AES-256-GCM Company/Contact SSN vault (`abDeriveVaultKey` et al, also in `AmitBooks\NEW.html`) — a short repeating-key shift cipher is breakable by hand given the ciphertext, unlike AES. Ryan's explicit, informed call to use it anyway for convenience on data like vendor Access/login fields. **Scope, as instructed 2026-09-17: PIN Encryption is for NEW restricted fields going forward — it does not replace or retrofit the existing SSN vault, which stays exactly as built, untouched, unless Ryan explicitly says otherwise for that specific field.** If a future session is asked to protect a new sensitive field and it's genuinely unclear whether Ryan wants the strong AES vault or the simpler PIN cipher for that specific case, ask — the two are not interchangeable by default, and this file's job is to prevent that ambiguity from being silently guessed wrong on real sensitive data.
