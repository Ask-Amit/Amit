...
**Session 58 (2026-07-28, in progress), AmitCoder moved from Hub placeholder to a real page + voice-shortcut convention discovered live.** Most of the session was spent testing, out loud, how to make a spoken/typed trigger word reliably recognizable through Ryan's actual microphone — several candidates (z, Selah, Shomer, Aleph, FJ/JF combos) were tested live and most failed in genuinely useful ways (single unstressed syllables get dropped, not misheard; two-letter combos spoken together transcribe wildly inconsistently). Landed on: a full word (Selah, or similar) to activate shortcut mode once per session, then single letters F/J as the fast in-session command triggers — both proven reliable in isolation and embedded mid-sentence. This produced AmitCoder as its real home: moved off the Hub's placeholder tile into `AmitCoder\AmitCoder.html`, a real standalone page with a deliberately different visual theme (dark/monospace "IDE" look) from the rest of Amit's warm gold styling. New Supabase table `amit_shortcuts` created (with `is_builtin`/`is_active` columns added same session) — same login as the Hub, no separate account system. Ryan authorized an open-ended unsupervised build session afterward: creating shortcuts, builtin-vs-custom UI, and additional placeholder tabs. See `AmitCoder\CLAUDE.md` for full current state — check it before assuming what's built vs. still planned, since this entry was written mid-session.
