...
**NEXT SESSION — first task:** write a shortcut (Ryan's explicit request) that stands up the whole Voice Control system — relay, watcher, supervisor, and the Monitor arm on the shared events log — in one step, instead of the many manual commands used to build and test it this session.
